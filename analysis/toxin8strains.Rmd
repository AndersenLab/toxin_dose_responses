...
fit_all_models <- function(data, traits = "median_wormlength_um_reg", ...){
  ...
  LL4_fit_data <- data %>%
    ...
    dplyr::filter(trait %in% traits,
                  concentration_um %in% complete.doses$concentration_um) %>% # filter to traits provided in arguments
    # dplyr::group_by(drug, strain, trait, Metadata_Experiment) %>% 
    dplyr::group_by(drug, strain, trait) %>% # should eventually be "..." passed from function + trait
    tidyr::nest() %>%
...
strain.EC50s <- function(drug.drm, drug.df){
  diditwork <- c(is.character(drug.drm$ED))
  ...
  EC.list <- list()
  for(i in 1:nrow(drug.drm)){
    
    stuff <- drug.drm$ED[[i]] %>%
      data.frame() %>%
      dplyr::mutate(drug = drug.drm[i,]$drug,
                    strain = drug.drm[i,]$strain,
                    trait = drug.drm[i,]$trait,
                    model = drug.drm[i,]$model)
    stuff$fraction <- rownames(stuff)
    rownames(stuff) <- NULL
    ECs <- stuff %>%
      dplyr::mutate(fraction = if_else(fraction == "e:1:10", true = "EC10", false = fraction),
                    fraction = if_else(fraction == "e:1:50", true = "EC50", false = fraction),
                    fraction = if_else(fraction == "e:1:90", true = "EC90", false = fraction))
...
library(ggtree)
library(ape)
CENDR.tree <- ape::read.tree(glue::glue("data/WI.20210121.hard-filter.isotype.min4.tree"))
CENDR.tree$tip.label[!CENDR.tree$tip.label %in% c("N2","CB4856","ECA396","ECA36","XZ1516","MY16","RC301","CB4855")] <- ""

CENDR.ggplot.tree <- as.ggplot(function() plot(CENDR.tree, 
                          type = "u",
                          show.tip.label = T, edge.width = 0.5, no.margin = TRUE))
reduced.8strain.CeNDR.tree <- CENDR.ggplot.tree + 
  theme(plot.background = element_rect(fill = "white",colour = NA))
ggsave(reduced.8strain.CeNDR.tree, filename = "output/reduced.8strain.CeNDR.tree.png", width = 7, height = )
...
# silver.df[[1]] %>%
#   dplyr::mutate(tx_stock = if_else(Metadata_Experiment == "toxin28A", 
#                                    true = "20210626", 
#                                    false = "2019")) %>%
#   ggplot(.) + 
#   theme_bw() +
#   geom_quasirandom(mapping = aes(x = strain, 
#                                  y = median_wormlength_um_reg, 
#                                  colour = Metadata_Experiment), size = 1) + 
#   geom_boxplot(mapping = aes(x = strain, 
#                              y = median_wormlength_um_reg, 
#                              fill = tx_stock), 
...
#        title = "Silver Nitrate - Stock Comparison")
# ggsave(filename = "output/silver.nitrate.stocks.png", height = 6, width = 8)
...
silver.LL4.estimates <- strain.EC50s(drug.drm = drug.drm,
                                    drug.df = silver.df[[1]])
EC10.plot(e = silver.EC.estimates, 
          d = silver.df)
```
...
#                           drug.df = cadmium.df[[1]])
EC10.plot(e = cadmium.LL4.estimates, d = cadmium.df, basesize = 16)
```
...
EC10.plot(e = copper.LL4.estimates, d = copper.df)
```
...
#                           drug.df = nickel.df[[1]])
EC10.plot(e = nickel.LL4.estimates, d = nickel.df)
```
...
#                           drug.df = paraquat.df[[1]])
EC10.plot(e = paraquat.LL4.estimates, d = paraquat.df)
...
#                           drug.df = zinc.df[[1]])
EC10.plot(e = zinc.LL4.estimates, d = zinc.df)
```
...
#                           drug.df = pyraclostrobin.df[[1]])
EC10.plot(e = pyraclostrobin.LL4.estimates, d = pyraclostrobin.df, basesize = 16)
```
...
aldicarb.df[[1]] %>%
  ...
  labs(x = "Strain",
       y = "Relative Animal Length",
       title = "Aldicarb - Stock Comparison")
ggsave(filename = "output/aldicarb.stocks.png", height = 6, width = 8)
...
#                           drug.df = aldicarb.df[[1]])
EC10.plot(e = aldicarb.LL4.estimates, d = aldicarb.df)
```
...
#                           drug.df = chlorfenapyr.df[[1]])
EC10.plot(e = chlorfenapyr.LL4.estimates, d = chlorfenapyr.df, basesize = 16)
...
#                           drug.df = methomyl.df[[1]])
EC10.plot(e = methomyl.LL4.estimates, d = methomyl.df)
```
...
methyl.mercury.LL4.estimates %>%
  dplyr::filter(strain == "PD1074") %>%
  dplyr::select(strain, fraction, Estimate)
...
#                           drug.df = methyl.mercury.df[[1]])
EC10.plot(e = methyl.mercury.LL4.estimates, d = methyl.mercury.df)
```
...
as.dose.check.plot <- dose.check.plot(arsenic.df[[1]])
ggsave("output/JAX.as.dose.check.plot.png", width = 6, height = 6)

arsenic.df[[1]] %>%
  dplyr::select(Metadata_Experiment, Metadata_Plate, strain, Metadata_Well, concentration_um) %>% 
  dplyr::distinct() %>%
  dplyr::filter(Metadata_Experiment == "toxin17A",
                concentration_um == 418.6) %>%
  data.frame()
...
#                           drug.df = arsenic.df[[1]])
EC10.plot(e = arsenic.LL4.estimates, d = arsenic.df)
```
...
#                           drug.df = carbaryl.df[[1]])
EC10.plot(e = carbaryl.LL4.estimates, d = carbaryl.df)
```
...
#                                                         drug.df = carboxin.df[[1]])
EC10.plot(e = carboxin.LL4.estimates, d = carboxin.df)
```
...
#                           drug.df = chlorpyrifos.df[[1]])
EC10.plot(e = chlorpyrifos.LL4.estimates, d = chlorpyrifos.df)
```
...
#                           drug.df = lead.df[[1]])
EC10.plot(e = lead.LL4.estimates, d = lead.df)
```
...
#                           drug.df = atrazine.df[[1]])
EC10.plot(e = atrazine.LL4.estimates, d = atrazine.df)
```
...
x2.4.D.df[[1]] %>%
  ...
  labs(x = "Strain",
       y = "Relative Animal Length",
       title = "2,4-D - Stock Comparison")
ggsave(filename = "output/24-D.stocks.png", height = 6, width = 8)
...
#                           drug.df = x2.4.D.df[[1]])
EC10.plot(e = x2.4.D.LL4.estimates, d = x2.4.D.df)
```
...
# Malathion_DRCplot +
#   ggplot2::labs(subtitle = "Selected Models")
# malathion.selected.EC.estimates <- selected.strain.EC50s(best.drug.drm = best.drug.drm,
#                           drug.df = malathion.df[[1]])
EC10.plot(e = malathion.LL4.estimates, d = malathion.df)
```
...
#                           drug.df = chlorothalonil.df[[1]])
EC10.plot(e = chlorothalonil.LL4.estimates, d = chlorothalonil.df)
```
...
# Manganese_dichloride_DRCplot +
#   ggplot2::labs(subtitle = "Selected Models")
# manganese.selected.EC.estimates <- selected.strain.EC50s(best.drug.drm = best.drug.drm,
#                           drug.df = manganese.df[[1]])
EC10.plot(e = manganese.LL4.estimates, d = manganese.df)
```
...
#                           drug.df = propoxur.df[[1]])
EC10.plot(e = propoxur.LL4.estimates, d = propoxur.df)
```
...
#                                                         drug.df = mancozeb.df[[1]])
EC10.plot(e = mancozeb.LL4.estimates, d = mancozeb.df)
```
...
bacteria.df <- Reduce(rbind,bacteria) %>%
  ...
  dplyr::filter(cv.censor != 1) %>%
  cleanData(.)
data.cleaning.plot(bacteria.df[[2]], "Bleach Quality")
dose.check.plot(bacteria.df[[1]]) + 
  ggplot2::scale_y_continuous(limits = c(-100, 600))

fit_best_models_BACTERIA <- function(data, traits = "median_wormlength_um_reg", ...){
  # extract concentration variable name
  conc_var_name <- str_subset(names(data), pattern = "concentration_um")
  # dose-response model inference
  fit.best.model  <- function(x, model){
    ...
    if(model == "W1.4"){
      drc::drm(data = x, fct = W1.4())
    } else if(model == "LL.4"){
      drc::drm(data = x, fct = LL.4())
    } else if(model == "LN.4"){
      drc::drm(data = x, fct = LN.4())
    } else if(model == "W2.4"){
      drc::drm(data = x, fct = W2.4())
    } else {
...
plot_best_DRC_BACTERIA <- function(all.model.list, toxin, metadata){
...
best.drug.drm <- suppressMessages(fit_best_models_BACTERIA(data = bacteria.df[[1]], 
                                                           traits = "median_wormlength_um_reg"))
plot_best_DRC_BACTERIA(all.model.list = best.drug.drm,
                       toxin = levels(as.factor(bacteria.df[[1]]$drug)), 
                       metadata = bacteria.df[[1]])
Bacteria_OD_DRCplot
```

## EC Estimates

```{r DRC metrics, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
complete.LL4s.EC10 <- LL4.EC.est.dat %>%
  dplyr::filter(fraction == "EC10") %>%
  dplyr::select(drug, Estimate, Strain, Std..Error, class, null.estimate, above.max.conc)

complete.LL4s.EC50 <- LL4.EC.est.dat %>%
  dplyr::filter(fraction == "EC50") %>%
  dplyr::select(drug, Estimate, Strain, Std..Error, class, null.estimate, above.max.conc)

complete.DRs.EC10 <- complete.LL4s.EC10 %>%
  dplyr::group_by(drug,above.max.conc) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = above.max.conc, values_from = `n()`) %>%
  dplyr::filter(`EC Estimate Within Dose Range` == 8)

complete.DRs.EC50 <- complete.LL4s.EC50 %>%
  dplyr::group_by(drug,above.max.conc) %>%
  dplyr::summarise(n()) %>%
  tidyr::pivot_wider(names_from = above.max.conc, values_from = `n()`) %>%
  dplyr::filter(`EC Estimate Within Dose Range` == 8)
...
complete.DRs.EC50 %>%
  dplyr::left_join(., complete.LL4s.EC50) %>%
# complete.LL4s.EC50 %>%
  dplyr::select(drug, Strain, Estimate, Std..Error) %>%
  ggplot(.) + 
  theme_bw() + 
  geom_pointrange(mapping = aes(x = Strain, y = Estimate, ymax = Estimate+Std..Error, ymin = Estimate-Std..Error)) + 
  facet_wrap(.~drug, scales = "free_y") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        panel.grid = element_blank()) + 
  labs(y = "EC50 Estimate")

all8.EC10s <- complete.DRs.EC10 %>%
  dplyr::left_join(., complete.LL4s.EC10) %>%
  dplyr::select(drug, Strain, Estimate, Std..Error) %>%
  ggplot(.) + 
  theme_bw() + 
  geom_pointrange(mapping = aes(x = Strain, y = Estimate, ymax = Estimate+Std..Error, ymin = Estimate-Std..Error)) + 
  facet_wrap(.~drug, scales = "free_y", nrow = 3) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
        panel.grid = element_blank()) + 
  labs(y = "EC10 Estimate")
ggsave(all8.EC10s, width = 10, height = 6, filename = "output/all8.EC10s.png")
  

complete.LL4s.EC10 <- LL4.EC.est.dat %>%
  dplyr::filter(fraction == "EC10",
                drug != "Deltamethrin",
                null.estimate == "Estimated ECs") %>%
  dplyr::select(drug, Estimate, Strain, class, above.max.conc)


LL4.EC.est.dat$class[grep(LL4.EC.est.dat$class, pattern = "Carbamate Insecticide")] <- "Insecticide"
LL4.EC.est.dat$class[grep(LL4.EC.est.dat$class, pattern = "Organophosphate Insecticide")] <- "Insecticide"
LL4.EC.est.dat$class[grep(LL4.EC.est.dat$class, pattern = "Synthetic Pyrethroid Insecticide")] <- "Insecticide"

EC10.range <- LL4.EC.est.dat %>%
  dplyr::filter(fraction == "EC10",
                !drug %in% c("Deltamethrin","Manganese dichloride")) %>%
  dplyr::group_by(class) %>%
  ggplot(., mapping = aes(y = drug, x = log(Estimate), colour = Strain)) +
  theme_bw(base_size = 12) +
  # geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(shape = above.max.conc), height = 0.05) + 
  scale_colour_manual(values = strain_colors) + 
  scale_shape_manual(values = c(19,1)) + 
  theme(panel.grid = element_blank(),
        strip.text.y = element_text(angle = 0)) + 
  facet_grid(class ~ ., scales = "free", space = "free") + 
  labs(x = expression(log[10](EC10)),
       y = "Compound")
EC10.range + theme(legend.position = "none")
ggsave(EC10.range+ theme(legend.position = "none"), width = 10, height = 6, filename = "output/EC10.range.png")


EC10.comps <- LL4.EC.est.dat %>%
  dplyr::filter(fraction == "EC10",
                drug != "Deltamethrin") %>%
  ggplot(., mapping = aes(x = Strain, y = Estimate, fill = Strain)) +
  theme_bw(base_size = 10) +
  geom_point(shape = 21, size = 3) + 
  geom_pointrange(shape = 21, aes(ymin = Lower, ymax = Upper)) +
  ...
  theme(axis.text.x = element_blank(),
        ...
        panel.grid = element_blank()) +
  labs(x = "Toxicant")
ggsave(EC10.comps, width = 13, height = 8, filename = "output/EC10.comps.png")


all.DRC.plot <- cowplot::plot_grid(
  plotlist = list(Cadmium_chloride_DRCplot, Arsenic_trioxide_DRCplot, `Copper(II)_chloride_DRCplot`, 
                  `Lead(II)_nitrate_DRCplot`, Methylmercury_dichloride_DRCplot,Nickel_chloride_DRCplot,
                  Silver_nitrate_DRCplot, Zinc_chloride_DRCplot,
                  
                  `2,4-D_DRCplot`, Atrazine_DRCplot, Paraquat_DRCplot,
                  
                  Aldicarb_DRCplot, Carbaryl_DRCplot, Methomyl_DRCplot, Propoxur_DRCplot,
                  
                  Chlorfenapyr_DRCplot, Chlorpyrifos_DRCplot, Malathion_DRCplot,
                  
                  Carboxin_DRCplot, Chlorothalonil_DRCplot, Mancozeb_DRCplot, Pyraclostrobin_DRCplot,
                  
                  Triphenyl_phosphate_DRCplot
                  )
)

ggsave(all.DRC.plot, width = 13, height = 8, filename = "output/allDRCplot.png")

```

### EC10

```{r EC10 plots, eval=FALSE, fig.height=7, fig.width=10, message=FALSE, warning=FALSE, include=FALSE}
h2.doses <- data.table::fread("data/h2doses.csv")
tox.dfs <- ls(pattern = "df")
tox.dfs <- tox.dfs[!tox.dfs %in% c("tox.dfs","conc.mean.df")]
max.responses <- list()
for(i in 1:length(tox.dfs)){
  
  phenos <- get(tox.dfs[i])[[1]] %>%
    dplyr::select(drug, strain, concentration_um, median_wormlength_um_reg)
  
  complete.responses.concentrations <- phenos %>%
    dplyr::group_by(concentration_um, strain) %>%
    dplyr::summarise(n = n()) %>%
    dplyr::filter(n > 10) %>%
    dplyr::ungroup() %>%
  ...
  dose <- h2.doses %>%
    dplyr::filter(drug == unique(phenos$drug)) %>%
    dplyr::select(concentration_um) %>%
    as.numeric()
  
  concs <- c(min(complete.responses.concentrations$concentration_um),
             round(dose,1))
  
  ctrl <- phenos %>%
    dplyr::filter(concentration_um %in% concs[1]) %>%
    dplyr::group_by(concentration_um) %>%
    dplyr::summarise(mean(median_wormlength_um_reg))
  
  conc.mean <- phenos %>%
    dplyr::filter(concentration_um %in% concs[2],
                  !is.na(median_wormlength_um_reg)) %>%
    dplyr::group_by(concentration_um) %>%
    dplyr::summarise(conc.mean = mean(median_wormlength_um_reg))

  max.responses[[i]] <- phenos %>%
    dplyr::filter(concentration_um %in% concs[2],
                  !is.na(median_wormlength_um_reg)) %>%
    dplyr::group_by(concentration_um, strain) %>%
    dplyr::summarise(mean = mean(median_wormlength_um_reg),
                     sd = sd(median_wormlength_um_reg)) %>%
    dplyr::mutate(ctrl = ctrl$`mean(median_wormlength_um_reg)`,
                  delta = mean - ctrl,
                  drug = unique(phenos$drug),
                  conc.mean = conc.mean$conc.mean)
  max.responses[[i]]
}
max.responses.dat <- max.responses %>%
  dplyr::bind_rows(.) %>%
  dplyr::left_join(.,drug.classes) %>%
  dplyr::mutate(dose = paste0(drug,"; ",concentration_um," µM"),
                drug = as.factor(drug), class = as.factor(class))
class.palette <- c("brown","steelblue1","darkgreen","navajowhite","tan","sandybrown","yellow","red")
names(class.palette) <- unique(max.responses.dat$class)
conc.mean.dat <- max.responses.dat %>%
  dplyr::ungroup() %>%
  dplyr::select(dose, class, conc.mean) %>%
  distinct()
big.tox.plot <- ggplot(max.responses.dat) + 
  theme_bw(base_size = 24) +
  geom_pointrange(mapping = aes(x = -mean, 
                                y = reorder(dose,mean), 
                                xmin = -mean-sd, 
                                xmax = -mean+sd,
                                colour = strain),
                  position = position_dodge2(width = 0.5), size = 1.25) +
  scale_colour_manual(values = strain_colors, name = "Strain") + 
  # geom_label_repel(data = conc.mean.dat, aes(x = -conc.mean,
  #                                      y = reorder(dose,conc.mean), 
  #                                      label = round(conc.mean,2), fill = class), 
  #            nudge_x = 200, nudge_y = 3, size = 7, box.padding = unit(0.5, "lines")) + 
  scale_fill_manual(values = class.palette, guide = F) + 
...
width = 10
height = width*2.5
ggsave(big.tox.plot, filename = "output/big.tox.plot.png", width = width, height = height)
  


big.zinc <- Zinc_dichloride_DRCplot +
  theme_bw(base_size = 20) + 
  theme(legend.position = "none",
        axis.title.y = element_blank())

big.atrazine<- Atrazine_DRCplot +
  theme_bw(base_size = 20) + 
  theme(legend.position = "none",
        axis.title.y = element_blank())

big.carbaryl <- Carbaryl_DRCplot +
  theme_bw(base_size = 20) + 
  theme(legend.position = "none",
        axis.title.y = element_blank())

big.chlorf <- Chlorfenapyr_DRCplot +
  theme_bw(base_size = 20) + 
  theme(legend.position = "none",
        axis.title.y = element_blank())
bigDRC.grid <- cowplot::plot_grid(big.zinc, big.atrazine, big.carbaryl, big.chlorf, ncol = 2)
ggsave(bigDRC.grid, filename = "output/bigDRC.grid.png", width = 10, height = 10)
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE, include=FALSE, eval=FALSE}
...
```{r carbamate insecticides, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
drug.classes %>%
...
```{r fungicides, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
drug.classes %>%
...
```{r pyrethroids, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
drug.classes %>%
...
```{r organophosphate, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
drug.classes %>%
...
```{r metal, echo=FALSE, message=FALSE, eval=FALSE, warning=FALSE, include=FALSE}
drug.classes %>%
...
```{r herb, echo=FALSE, message=FALSE, eval=FALSE, warning=FALSE, include=FALSE}
drug.classes %>%
