...
sampling.plot <- function(data){
  data %>%
  dplyr::group_by(strain, bleach, Metadata_Experiment, concentration_um) %>%
  dplyr::summarise(n.wells = n()) %>%
  tidyr::unite("assay_bleach", Metadata_Experiment:bleach) %>%
  ggplot(mapping = aes(x = assay_bleach, y = n.wells, fill = strain)) + 
  theme_bw() + 
  geom_hline(yintercept = 30, linetype = 3) + 
  geom_col() + 
  facet_wrap(.~concentration_um) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) + 
  labs(x = "Independent Bleaches",
       y = "Number of Wells",
       title = unique(data$drug))
}
```
...
# 3 Model
# all.3model.dat <- processed.toxin09A$summarized_processed %>%
...
#   dplyr::full_join(processed.toxin23A$summarized_processed) %>%
...
# 
# # Percentage of Expected Wells - 3 Model
# raw.tab.dat %>%
#   dplyr::filter(n < 30,
#                 n > 5) %>%
...
#   labs(x = "Strain", y = "Bleach", title = "3 Worm Model Measurements")+ 
#   ggsave(filename =  "output/3model.heatmap.png", height = 10, width = 15)
# 
...
`2,4-D_DRCplot`
x2.4.D.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
                                    drug.df = x2.4.D.df[[1]])
```
...
Malathion_DRCplot
malathion.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
...
chlorothalonil.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
...
deltamethrin.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
...
manganese.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
...
Propoxur_DRCplot
propoxur.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
...
Mancozeb_DRCplot
mancozeb.EC.estimates <- strain.EC50s(drug.drm = drug.drm, 
...
EC.est.dat %>%
  ...
  labs(x = "Toxicant", 
       y = "EC10 Estimate (µM)") + 
  ggsave("output/EC10.strain.comp.png", width = 12, height = 8)
```
