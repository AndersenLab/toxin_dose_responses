...
#                           drug.df = cadmium.df[[1]])
EC10.plot(e = cadmium.LL4.estimates, d = cadmium.df)
```
...
#                           drug.df = pyraclostrobin.df[[1]])
EC10.plot(e = pyraclostrobin.LL4.estimates, d = pyraclostrobin.df)
```
...
#                           drug.df = chlorfenapyr.df[[1]])
EC10.plot(e = chlorfenapyr.LL4.estimates, d = chlorfenapyr.df)
```
...
#                           drug.df = manganese.df[[1]])
# EC10.plot(e = manganese.selected.EC.estimates, d = manganese.df)
```
...
EC10.range <- LL4.EC.est.dat %>%
  dplyr::filter(fraction == "EC10",
                drug != "Deltamethrin") %>%
  ggplot(., mapping = aes(x = drug, y = log(Estimate))) +
  ...
  labs(x = "Toxicant",
       y = "log(EC10)")
ggsave(EC10.range, width = 13, height = 8, filename = "output/EC10.range.png")
...
all.DRC.plot <- cowplot::plot_grid(
  plotlist = list(Cadmium_dichloride_DRCplot, Arsenic_trioxide_DRCplot, `Copper(II)_chloride_DRCplot`, 
                  `Lead(II)_nitrate_DRCplot`, Methylmercury_dichloride_DRCplot,Nickel_dichloride_DRCplot,
                  Silver_nitrate_DRCplot, Zinc_dichloride_DRCplot,
