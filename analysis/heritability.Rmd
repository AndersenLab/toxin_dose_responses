...
complete.doses <- function(x){
  complete.dose.table <- x %>%
  ...
  complete.doses.df <- x %>%
    dplyr::filter(concentration_um %in% complete.dose.table$concentration_um)
  return(complete.doses.df)
}
H2 <- function(d){
...
DR.herit.summary <- function(H2.out, x, doses){
  ...
  population.10perceffect <- x %>%
    ...
    dplyr::select(avg.effects) %>% as.numeric() * tiles[i]
  ...
  frac.affected <- population.response %>%
    ...
    geom_line(aes(x=log(concentration_um), 
                  y=fraction.affected*100), size = 0.3) + 
    theme(panel.grid = element_blank()) +
    geom_vline(xintercept = log(GWA.dose.herits$concentration_um), linetype = 2, size = 0.2) + 
    ...
    labs(x=expression(italic("log")("Concentration (μM)")),
         y = paste0(tiles[i]*100,"%"))
  plot.list[[i]] <- frac.affected
  }
  strains <- ggplot() +
    theme_bw() + 
  ...
  DiM.plot <- plot_grid(plot.list[[1]],plot.list[[2]],plot.list[[3]], ncol = 1)
  
  y.grob <- textGrob("Population Exhibiting Response of x Effect (%)", 
                     gp=gpar(fontsize=10), rot = 90)
  DiM.plot.2 <- grid.arrange(arrangeGrob(DiM.plot,left = y.grob))
  
  summary.plot <- cowplot::plot_grid(strains, DiM.plot.2, rel_widths = c(3:2))
  ggsave(summary.plot + theme(plot.background = element_rect(fill = "white",colour = NA)), 
...
all.HH.plot <- ggplot(herit.summary, mapping = aes(x = value_H2, y = value_h2, colour = log(concentration_um))) + 
  theme_bw(base_size = 12) +
  geom_abline(slope = 1, alpha = 0.5, linetype = 4) +
  ...
  geom_errorbar(aes(ymin = lower_h2, ymax = upper_h2)) +
  geom_errorbarh(aes(xmin = lower_H2, xmax = upper_H2)) + 
  facet_wrap(.~drug) + 
  ylim(c(-0.05,1)) + 
  xlim(c(-0.05,1)) + 
  scale_colour_gradient2(low = "darkblue", mid = "violet", high = "darkred", midpoint = 2.5, 
  ...
  labs(x = bquote(H^2),
       y = bquote(h^2))
all.HH.plot
ggsave(all.HH.plot, filename = "output/all.HH.plot.png", width = 10, height = 7)
