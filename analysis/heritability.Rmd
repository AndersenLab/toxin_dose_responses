...
drugHeritability <- function(rep.data, concentration_um){
  ...
  if(1 %in% no.reps$n){
    ...
    herits <- suppressMessages(H2.bootstrapping.calc(d = rep.data, nreps = 100, boot = T))
...
```{r}
df_h <- drug.data.nested.reps$data[[8]] %>% ## find some way to integrate dosage and drug information, maybe keep purrr
  dplyr::select(strain, value, replicate)
...
narrowh2 <- function(df_h, geno_matrix){
  ...
  h2_combine = c(trait, h2, h2_SE) ## make trait drug and dose information
