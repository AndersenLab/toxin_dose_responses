...
raw.tab.dat %>%
  ...
  labs(x = "Strain", y = "Drug", title = "3 Worm Model Measurements")+ 
  ggsave(filename =  "output/3model.heatmap.png", height = 15, width = 15)
...
raw.tab.dat.MDHD %>%
  ...
  scale_fill_distiller(palette = "RdYlBu", direction = 1, name = "Assay Complement (% Expected Wells)") + 
  facet_grid(assay~bleach, drop = TRUE, scales = "free", space = "free") + 
  labs(x = "Strain", y = "Drug", title = "4 Worm Model Measurements, Worm_Length < 50") + 
  ggsave(filename = "output/4model.heatmap.png", height = 15, width = 15)
...
silver.df <- Reduce(rbind,silver) %>%
  ...
  dplyr::filter(!Metadata_Experiment %in% c("toxin09A","toxin15A")) %>%
  cleanData(.)
...
cadmium.df <- Reduce(rbind,cadmium) %>%
  ...
  dplyr::left_join(., cv.n.censor) %>%
  dplyr::filter(cv.censor != 1) %>%
  dplyr::filter(!Metadata_Experiment %in% c("toxin09A","toxin11A","toxin15A")) %>%
  cleanData(.)
data.cleaning.plot(cadmium.df[[2]], "Bleach Quality")
